...
def get_average_rating_value_by_book(db: Session, book_id: UUID):
    ...
    Returns:
        _type_: _description_
    """
    return db.query(func.avg(Rating.rating_value).label('average_rating_value'), func.count(Rating.book_id).label('total_ratings')).group_by(Rating.book_id).filter(Rating.book_id==book_id, Rating.is_reviewed).all()
...
def get_average_rating_statistic_by_book(db: Session, book_id: UUID):
    ...
    """
    try:
        data = db.query(Rating.rating_value, func.count(Rating.user_id).label('total_ratings')).group_by(Rating.rating_value).filter(Rating.book_id==book_id, Rating.is_reviewed).all()
        return True, data
    except Exception:
        return False, None

def get_book_comments(db: Session, book_id: UUID)-> list[BookRatingViewModel]:
    """ Get book comments
